# v15 — Constructor-controlled brands (seal scope)
...
included — breaks the flagship: `auth.ail` tests and scripts
`Db__Hash` values declared in `db.ail`, and cross-module test
...
One check in `checkTypes` (new code AIL6004): the tycker tracks
...
- The flagship compiles untouched: `db.ail` seals its own brand
  in its body, and every `auth.ail` seal sits in a test or script
...
- Negative examples ship with the change: `TestSealForeignBrandInBody`
  ...
  and `sketches/broken-login/foreign-seal.ail` carries the single
  AIL6004 squiggle (its provider gained the shared `Db__Hash`).
