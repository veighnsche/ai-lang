# v24 — Row 4 remainder (rounding, rendering, exact int, exact root)

Status: shipped. Row 4 of the standard-library program closes with
five operations over the v21 observation primitive. No new kernel,
...
- `std__convert__dec_to_int_exact(value)` → the integer or
  `convert.fractional_value`, reusing the v22 strip worker.
...
- The `int__pow` / `sqrt_floor` *entries* are never called from
  ...
  force an unreachable error arm (v22 rule).
...
- No compiler changes; no new Go tests (no new Go code path
  exists — the v21 kernel pins plus the tables plus the golden
  cover the slice).
