...
Status: shipped. Amendment to v04 (type discipline) and v06
...
v06 proved `dec` on scaled integers and `int` as int64-or-loud, then
...
`-`, `*` never wrap and never fail loud. `dec` keeps its v06
...
- `eval.go`: `Value.N` is `*big.Int`; the `IsInt64` gate is deleted;
  ...
  `big.Int`. (v11 deletes the `decreases` negativity checks as
  unreachable past the guard rule.)
- `check.go`: the step rule compares unbounded literals (same
  syntactic shape; v11 tightens it to the unit step `p - 1`).
...
- v04's "15 significant digits" boundary and v06's "TS mapping is
  exact for `|int| < 2^53`" are superseded; both carry an
  `Amendment (v10)` pointer to this doc. The v05 plan baseline is
  left as history.

## Open decisions (do not block)

- Resource exhaustion from unbounded growth (a host-memory question)
  is modeled nowhere yet — same class as the v08 stack boundary:
  documented, not solved.
- Division stays deferred under its v06 reason; this spec changes
  nothing about `/`.
