...
2. `std/text/text.ail`: `fn std__hex__decode(value: str) ->
   ...
   truncated `"a"`). No committed raw NUL/BOM: text.ail
...
4. Regen: `text.ts` gains the union member, the
   `$ailHexDecode` helper (first hex decode call site in
...
- Foreign-caller probe first (red): temp `client.ail` with
  ...
  (AIL4107): success + failure + NUL-input... NUL as INPUT
  ...
  .ail change (AIL2102), passes after.
