...
1. `B + B -> B` when `B` is a str-backed brand (checker
   ...
   plus concats. This narrows v16's conservative refusal with
...
But A is a new compiler kernel slice (v43+) AFTER the
verdict — this slice ships the fork-independent fragment
only, and `attributes__make` waits for the decision. The
v30/v31 join law (omit empties, separators between retained
