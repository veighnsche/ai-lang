# v51: std__utf8__decode wrapper (B7 scope)
...
1. `std/text/text.ail`: `provides += std__utf8__decode`. (`uses []`
   stays. Mod-level `emits` untouched per the `empty_separator`
   precedent: it is not an exhaustive inventory.)
2. `std/text/text.ail`: `fn std__utf8__decode(value: Bytes) ->
   ...
   input). No committed raw NUL/BOM: text.ail stays a text
...
4. Regen: `text.ts` gains the union member, the wrapper fn,
   AND the `$ailUtf8Decode` helper (kernel lowering inlines
