# v44 — attributes__make, customer 4 (shipped)
...
- v43 gives name-carrying items; emptiness is brand `==`
  against the empty seal (verified working); same-brand `+`
  (v42) assembles; `seals_from` promotion (v26,
...
- Catalogue: no first-wins/last-wins ambiguity; Collections +
  Brands (`docs/ASTRA_STDLIB.md:326`). Input shape amended by
  v43 verdict B (name-carrying items, not bare attributes).
...
- Risk: `seals_from` on a fresh brand looks like a back door.
  Mitigation: same-module, one-way, exact, declared — the v26
