...
- Brand `==` (erased, verified) and same-brand `+` (v42) are
  the only brand operators v44 needs beyond this slice.
...
1. Record (transparent, no validation needed — validity comes
   ...
   the duplicate rows in v44 use wrapper output only):
...
3. Boolean wrapper takes `(name, present)` like its maker;
   ...
   v44 omits them by the emptiness rule (name retained for
...
- Risk: record pairs invite hand-built mismatches. Accepted:
  ...
  language; the v44 rows use wrapper output, and no row blesses
  a hand pair.
...
- None. v44 owns `contains` + `make_from` + `make` +
