...
2. `std/text/text.ail`: `fn std__utf8__encode(value: str) ->
   ...
   precedent: héllo/世界); NO committed raw NUL — text.ail stays
   a text file, NUL relay is pinned by the Go test instead.
3. `std/text/text.ail` line 16 + `std/text/README.md`: retire the
...
4. `std/text/text.ts`: regen via `go run ./compiler --out std/text
   std/text/text.ail`. Expected diff: one union member plus one
...
- Foreign-caller test first (red): temp `client.ail` with
  ...
  that. Fails before the .ail change, passes after.
