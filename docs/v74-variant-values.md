...
[v73 registry](v73-variant-registry.md). Matching itself is v75.
...
- Equality boundary: `==`/`!=` over variants is refused at check
  time (like Bytes/Seq): cases compare by matching in v75, never
...
- Strict `tsc` (not lenient) gates emitted unions from this version
  on, per the v72 verdict.

## Out of scope

Matching/binding (v75), pilot consumers (v76), §2.3 identity beyond
