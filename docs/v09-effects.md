...
need no `given` table — the v07 principle extended to a third
callee kind.
...
(v07 locality precedent). Cells carry no `rev`: they are runtime,
...
table: there is nothing to script, exactly the v07 reasoning. A
...
tables — same class as the v04 int53 boundary: documented, not
solved. Async stays distant (v05): single-threaded store, no
ordering questions.

## Static rules (one rule, one code)

- `state__get`/`state__put` need no `uses` entry (same-file
  builtins, v07 precedent). Naming an undeclared cell is
...
- No build starts without approval of this doc; ratings and
  plans are not specs (v05 consequences still bind).
...
- Tests mirror the v08 shipment: diagnose cases per new code,
