# v30 — Boolean attribute gate plus presence/absence composition
...
- `html__attribute__boolean_name(value)` →
  ...
  singleton `disabled`, same shape as the v27 gate. Rejected
...
- `Html__Attribute` now holds zero or one complete approved
  ...
  v29 exactly-one reading. The future attributes-maker must
