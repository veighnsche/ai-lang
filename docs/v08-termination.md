...
together or not at all (v05); this is the together.
...
pre-execution gate as the v07 cycle ban, before anything runs or
...
> Amendment (v11): tightened — see `v11-recursion.md`. Only the
...
a DAG (v07), foreign calls never execute. Therefore every test
...
> Amendment (v11): superseded by the v11 theorem — see
> `v11-recursion.md`. Unit steps under the canonical guard make
...
> Amendment (v10): the int-overflow comparison is retired —
> unbounded ints have no overflow mode (`v10-numerics.md`). The
...
- A self-call site that does not pass `p - k` is not a loop step
  ...
  Amendment (v11): unit step only — the message now reads
  `pass p - 1`, and larger steps fail here too.
- A self-call site outside the positive branch is not admitted
  (`AIL3009`, v11): recursion must sit under the false arm of the
  canonical `p <= 0` guard.
- A self-call with no `decreases` line is the existing cycle error
  (`AIL3005`). That is the "would hang" shape, refused.
  Amendment (v11): the shape is finite either way — the refusal is
  proof-gated admission, not a diverted hang (`v11-recursion.md`).
...
- Grammar: `decreases` joins the control-keyword rule;
  `gramcheck` gains a sample, per v06 precedent.
...
(the JS stack), same class as the v04 int53 boundary — documented,
...
- `eval.go`: entry check (`p < 0` fails loud) in `evLocalCall`;
  ...
  Amendment (v11): the entry check is deleted at both routes (root
...
- `code.go`: `AIL3006`/`AIL3007`/`AIL3008` in the registry.
  Amendment (v11): plus `AIL3009` (unguarded recursion).

## Consequences (accepted before building)

- A new sketch, `sketches/retry-loop/`: retry-with-fuel against a
  ...
  Amendment (v11): the "would hang" claim was false and is
  corrected — the committed test now asserts proof-gated admission
  (`v11-recursion.md`). No golden changes: the shipped shape was
