...
| §1.8 `Map<K,V>` fully generic | ~~Nested generic Seq elements~~ — UNBLOCKED by b04: single-arg nesting already worked e2e via G2; the multi-arg literal-head comma split is fixed (`genericHeadLT` protects `<...>[` with nested angles) | Was: `seqElemName` rejects `<>` (stale — expansion stamps first); shipped str-keyed `Map__Entries<V>` instead. `Seq<Seq<..>>` stays rejected by design. |
