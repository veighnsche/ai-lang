# v46 S2: Owner-authorized typed UTF-8 export (B2)

Second execution slice of the Bytes workstream
([bytes-plan.md](/Users/vince/Projects/ai-lang/docs/bytes-plan.md) B2,
...
- Grant syntax `exports_utf8 Brand via fn@rev`: new top-level
  ...
  `via`/`@N`, surplus tokens, negative/overflowing revs are `AIL1000`,
...
- `certifyExports`: whole-program pass after world build, before any
  ...
  (`AIL6011`), and annotates the exact certified call node
  (`Small.ExportBrand`). Uncertified export calls are `AIL6010`.
- `Bytes__Value` compiler-owned record with coherent lookup
  ...
  `Bytes`/`Bytes__Value` and of the kernel name is `AIL6012`.
...
error kind — `AIL6010`/`AIL6011` are diagnostics, not `emits`).

## Rollback

Revert the slice commit. Nothing outside `compiler/` + grammar files +
this doc + the `docs/README.md` row moves. `AIL6010`/`AIL6011` stay
...
| E2 | Wrong scripted export bytes → `AIL3110`, both module orders | code |
| E3 | Grant removal → `AIL6010`, no stale certificate | code |
| E4 | Malformed grants → `AIL1000` | code |
| E5 | Cross-module / ambiguous / rev-mismatch / unknown-target grants → `AIL6010` | code |
| E6 | Shape violations (params, return, emits/effects/decreases, body, `given`, arms, RHS, arg spelling) → `AIL6011` | code |
| E7 | Export call without grant → `AIL6010` | code |
| E8 | Kernel-name and `Bytes__Value` shadowing → `AIL6012` | code |
...
| E12 | Same-basename two-directory attack → `AIL6010`, both orders | code |
