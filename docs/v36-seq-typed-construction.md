...
- Brand ownership preserved inside literals: executable positions mint
  ...
  name any declared brand as checked data (existing v15 rule).
...
- Precedents: v28 (this file's shape: P0 first, rollback section,
  open questions); v15 (brand ownership); v18 (scripted-not-executed
...
4. `vEq` gains structural Seq comparison (length + ordered element
   ...
   without it the new rows fall through the v18 "cannot compare,
...
6. Bare-Seq returns are rejected now (T8, mirroring the v26
