...
`1e3`) is a compile error, `AIL6001`, at the token, naming the legal
spelling:
...
the quotes (`d"abc"`, `d"1"`) is `AIL6001` with the same message.
...
TypeScript mapping: `dec` emits as `number`. Documented boundary: ailc
...
> Amendment (v10): superseded — see `v10-numerics.md`. `int` emits as
> `bigint` (unbounded, exact) and `dec` as canonical-digit strings
> through exact `$ailDec` helpers; no `number` mapping and no
...
(`AIL2002` on violation), carry `rev` like types, and are listed in
...
is `AIL6003`. The brand must be declared (`AIL6002`). There is no
...
brand. `reason = pw` where `reason: str` is `AIL6003`. Comparing
`pw == user.pw_hash` across two brands is `AIL6003`. Error fields,
...
- Positional call arg `i` against callee param `i` (ail or extern);
  arity mismatch → `AIL6003`. (Positional calls stay legal per open
...
- `sketches/auth-login/auth.ail`: declares `brand Auth__Password is
