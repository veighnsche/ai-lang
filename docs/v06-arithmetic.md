...
cost on the v05 list. Three operators, exact everywhere, one deferral
...
> Amendment (v10): superseded — see `v10-numerics.md`. `int` is now
...
## Type rule (extends the v04 binop rule)
...
`number`. The v04 boundary now covers both worlds explicitly: ailc
...
> Amendment (v10): superseded — see `v10-numerics.md`. `int` emits
...
- Whether `int64` overflow should be a static range error instead of
  ...
  Amendment (v10): moot — overflow no longer exists (unbounded ints).
- Larger integer model (leans: no; `int64`-or-loud is honest, bignum
  emit waits for the runtime proposal).
  Amendment (v10): decided — unbounded ints with `bigint` emit
  (`v10-numerics.md`).
