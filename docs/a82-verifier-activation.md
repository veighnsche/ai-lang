...
- `ailc baseline` generation is unchanged: generation is not
...
None. Inventory before wiring: no shipped `.ail` carries
`requires`/`ensures` (html.ail's match is the word
...
programs: without a solver those programs report AIL4305
...
- 5 durable probes in compiler/contract_activate_test.go:
  ...
  squiggles AIL4304, broken programs gain no AIL43xx noise,
