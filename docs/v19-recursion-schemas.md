# v19 — Blessed recursion schemas (spec)

Status: shipped. Amendment to v11 (unit loop): two more canonical
shapes are admitted, each pairing one step spelling with one guard
spelling, each with its theorem written down. The v11 rule is
...
1. `decreases p` — unit loop (v11, unchanged): every self-call site
