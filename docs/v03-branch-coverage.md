...
both are compile errors, `AIL4107`, at the arm's row, naming the pattern:
...
test in the world: the existing stale-emits diagnostic (`AIL4003`) is
...
- `checkSem` builds one coverage map per function, runs its tests
  through it, then emits `AIL4107` per untaken arm. Shared by editor
  and CLI; no separate CLI path.
- `allCodes` gains `CodeArmUntaken = "AIL4107"`.
- Stale-emits: severity warning → error in `checkEmits` only.

## Consequences (accepted before building)

- `sketches/auth-login/auth.ail` does not comply today: the inner retry
...
- `broken-login/stale-emits.ail` turns red (was yellow); its header
  comment is updated. `foreign-raise.ail` and `dead-script.ail` gain
...
- JSON golden gains no new lines from existing fixtures (all their arms
  are taken); the `AIL4003` line changes severity warning → error.
