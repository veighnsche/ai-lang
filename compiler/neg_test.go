...
// emit shapes ((-value), $ailDecSub("0.0", value)), no unary
...
// (-value), decimals reuse $ailDecSub("0.0", value) and never
// JavaScript -x.
...
`
	...
	ts := string(raw)
	for _, want := range []string{"((-a) * b)", `$ailDecSub("0.0", x)`} {
...
`
	...
	for _, want := range []string{"(-3n)", `$ailDecSub("0.0", "0.5")`} {
		if strings.Contains(ts, want) {
...
`
	...
	if diags := diagnose(dir, "m.ail", src); !hasCode(diags, "AIL6003") {
		t.Fatalf("mistyped negation reported no AIL6003: %v", diags)
	}
}
