...
- `db.ail` — `mod db`: user lookup + user type, own tests.
- `auth.ail` — `mod auth`: login with one retry, pins `db__get_user@5`
...
- `db.ts` + `auth.ts` — committed golden TS prod emits (tests/given
  ...
  sketches/auth-login/db.ail sketches/auth-login/auth.ail`; verify:
  `go test ./...`.
