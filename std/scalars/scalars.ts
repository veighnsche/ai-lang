...
export function std__dec__strip_from(remaining: bigint, current: bigint, exact: boolean): ScalarsResult {
  if ((remaining <= 0n)) {
    ...
    if (($ailDivMod(current, 10n)[1] === 0n)) {
      const $ail_m34: ScalarsResult = std__dec__strip_from((remaining - 1n), $ailDivMod(current, 10n)[0], exact);
      ...
      const $ail_m35: ScalarsResult = std__dec__strip_from((remaining - 1n), $ailDivMod(current, 10n)[0], false);
...
export function std__dec__truncate(value: string): ScalarsResult {
  const $ail_m36: ScalarsResult = std__dec__parts(value);
  switch ($ail_m36.$ail_kind) {
  case "ok": {
    const p = $ail_m36;
    const $ail_m37: ScalarsResult = std__dec__strip_from(p.scale, p.coefficient, true);
    switch ($ail_m37.$ail_kind) {
    case "ok": {
      const s = $ail_m37;
      if (s.exact) {
        const $ail_m38: ScalarsResult = std__convert__int_to_dec(s.stripped);
        ...
        if ((s.stripped >= 0n)) {
          const $ail_m39: ScalarsResult = std__convert__int_to_dec(s.stripped);
          ...
          const $ail_m40: ScalarsResult = std__convert__int_to_dec((s.stripped + 1n));
...
export function std__dec__floor(value: string): ScalarsResult {
  const $ail_m41: ScalarsResult = std__dec__parts(value);
  switch ($ail_m41.$ail_kind) {
  case "ok": {
    const p = $ail_m41;
    const $ail_m42: ScalarsResult = std__dec__strip_from(p.scale, p.coefficient, true);
    switch ($ail_m42.$ail_kind) {
    case "ok": {
      const s = $ail_m42;
      const $ail_m43: ScalarsResult = std__convert__int_to_dec(s.stripped);
...
export function std__dec__ceil(value: string): ScalarsResult {
  const $ail_m44: ScalarsResult = std__dec__parts(value);
  switch ($ail_m44.$ail_kind) {
  case "ok": {
    const p = $ail_m44;
    const $ail_m45: ScalarsResult = std__dec__strip_from(p.scale, p.coefficient, true);
    switch ($ail_m45.$ail_kind) {
    case "ok": {
      const s = $ail_m45;
      if (s.exact) {
        const $ail_m46: ScalarsResult = std__convert__int_to_dec(s.stripped);
        ...
        const $ail_m47: ScalarsResult = std__convert__int_to_dec((s.stripped + 1n));
