...
with its version (v04–v09). Rules are never rewritten silently.
...
- `state` cells (`state Name: T = lit`, v09) are module-private storage
...
- Every function declares `emits [...]`. Raising anything else = error.
  Entries are a conservative upper bound (v12): unrealized entries are
...
- State authority is declared beside `emits`: `effects [C.read, C.write]`
  (v09), transitive through local calls with no inference. Undeclared
...
- Iteration must provably halt before anything runs or emits (v08,
  v11): direct self-recursion only, program-wide — same-file and
...
- Each test evaluates with a fresh store built from `state` inits (v09);
...
- Every row is an exchange binding expected call args to one permitted
  outcome (v12): the table proves "this request received this permitted
...
- Error expectations are complete constructions
  ...
  Bare error kinds prove nothing about the payload and are errors (v12).
...
- Three callee kinds (v07, v09): foreign calls (ail via `uses`, externs)
...
- `ailc lsp` serves editor squiggles over stdio. Every keystroke
  ...
  undeclared emits entries), store authority (v09: undeclared `AIL3107`, stale
  ...
  (v10: unbounded ints, no overflow mode), no division) extends the
...
- Mapping: `emits` unions -> discriminated unions; proven-exhaustive
  ...
  -> module-scope `let`s with synthetic ok-unions at access sites (v09);
  numerics -> `bigint` ints and canonical-digit-string decs with exact
  `$ailDec` helpers emitted inline only where used (v10: no imports,
...
3. Helper-extraction rules — decided (v07): same-file callees need no
